...
def inference(net, args):
    ...
    seg_thr = [0.0]
    for thr in seg_thr:
        print("\n# seg_thr = " + str(thr), "  function = ", args.function, " mean_num = ", args.mean_num)
        args.seg_thr = thr
        local_acc0_sum = 0
        correct0_sum = 0
        local_acc5_sum = 0
        ...
        with torch.no_grad():
            for i, dat in enumerate(test_loader):
                ...
                # down
                pred_bbox = net.compute_rois_down(seg_thr=args.seg_thr, topk=cfg.top_k, combination=args.combination)
                # compute gt
                pred_gt_bbox = net.compute_gt_rois_down(seg_thr=args.seg_thr, labels=label_numpy, combination=args.combination)
